...
| ----------- | ----------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `apps/`     | what **humans** see                 | public UI                        | `@zippy/web` (Next.js), `@zippy/landing` (marketing), `@zippy/mobile` (Expo/React Native)                                     |
| `services/` | what has a **URL** / its own deploy | served to other code             | `@zippy/api` (Hono + OpenAPI), `@zippy/payment` (Creem adapter), `@zippy/ai-worker` (background, no URL)                      |
...
| `packages/` | what you **ship** — a distributable | served to **third parties**      | `@zippy/widget` (embeddable widget: IIFE `<script src>` + ESM); npm SDKs and CLIs live here too                               |
...
builders-stack/
├── apps/
│   ├── web/          @zippy/web       Next.js App Router — renders @zippy/ui, live Better Auth login
│   ├── landing/      @zippy/landing   public marketing site (@zippy/ui hero + shared <Analytics/>)
│   ├── blog/         @zippy/blog      static MDX blog — the GEO showcase, passes check:seo
│   └── mobile/       @zippy/mobile    real Expo / React Native starter rendering shared @zippy/ui tokens
├── services/
│   ├── api/          @zippy/api       Hono + OpenAPI (/health, /docs) — validates @zippy/api-types, mounts Better Auth
│   ├── payment/      @zippy/payment   Creem adapter + Mock provider + webhooks (/checkout)
│   └── ai-worker/    @zippy/ai-worker background load, NO URL (queue worker)
├── libs/
│   ├── ui/           @zippy/ui        shadcn components + tokens (web + RN) + Storybook
│   ├── auth/         @zippy/auth      Better Auth config (boot-verified end to end)
│   ├── db/           @zippy/db        Drizzle schema + client (the single ORM)
│   ├── ai/           @zippy/ai        provider-agnostic model client (Vercel AI SDK)
│   ├── analytics/    @zippy/analytics <Analytics/> provider + isomorphic typed event catalog (./events)
│   ├── email/        @zippy/email     Resend + React Email: typed, previewable templates + sendEmail()
│   ├── config/       @zippy/config    typed env: one Zod schema + cached getEnv()
│   ├── api-types/    @zippy/api-types the shared API contract (Zod schemas + inferred types)
│   └── seo/          @zippy/seo       the one door for page metadata + JSON-LD (enforced by check:seo)
├── packages/         what you SHIP (not run) — distributables served to third parties
│   └── widget/       @zippy/widget    embeddable feedback widget: IIFE (<script src>) + ESM build; type:package, libs-only, terminal
...
2. **One public door per lib.** Each lib exposes a single `src/index.ts`. Import by **package name** (`@zippy/db`), never a deep path (`@zippy/db/src/schema/users`). The barrel file is the contract.
...
4. **One ORM: Drizzle.** All DB access goes through `@zippy/db`. No raw `pg`, no second ORM.
5. **Payments through the adapter.** Never call Creem (or any provider) directly from an app or the API — go through `@zippy/payment`. Swapping providers should touch one file, not fifty.
6. **Config, not hardcoding.** No hardcoded URLs, ports, or secrets. Read typed env through `@zippy/config`'s `getEnv()` (backed by `.env.local`, see `.env.example`). Portless injects ports — nothing is pinned.
...
8. **SEO/GEO through `@zippy/seo` — enforced.** Every public page must export `metadata`/`generateMetadata` via `@zippy/seo`'s `pageMetadata()`, and public content must be server-rendered. `bun run check:seo` (in `bun run check`, lefthook pre-push, and CI) **fails the build** otherwise. Never hand-roll `Metadata`/OG/canonical or inline JSON-LD — see § 3.1 below.
9. **Packages are terminal.** A `packages/*` (tag `type:package`) is a **distributable you ship** (§1) — it may depend on `libs/*` **only** (not apps, services, or other packages), and **nothing internal may import it**. The tag matrix enforces both halves: `{ type:package → [type:lib] }`, and no other bucket lists `type:package` in its allowed tags, so an app/service/lib that imports `@zippy/widget` **fails `lint`**. A package is a leaf that leaves the repo — never a dependency inside it.
10. **`ops/` is the top of the graph — nothing imports from it.** `ops/` (deploy · db · secrets · runbooks · ci) is the **operate** layer: it reaches _down_ into the code to deploy/migrate/seed/provision it, and **no `app`/`service`/`lib`/`package` may reference `ops/*`**. It isn't a workspace, so `@zippy/*` resolution can't reach it and Nx has no target there — the boundary is structural, not just a rule. Secret provisioning under `ops/secrets/` points to **Ringtail** and nothing else. Brand/strategy is **not** here — that's `docs/brand/`. See `ops/README.md`.
11. **Storybook-first UI.** Every reusable UI element is a `@zippy/ui` component (bespoke ones too) — apps **compose**, never inline reusable UI or duplicate styles. A component is **incomplete** until it's in `libs/ui`, **has a Storybook story**, AND is used in an app (in the design system, in Storybook, _and_ used — all three). And **every new screen/flow ships a Storybook demo:** build its view as a presentational component driven by swappable `mock-*` state and story it, so the _whole screen_ is reviewable in Storybook with **no backend or keys** — the app page then only wires data + composes that view. (Because a `lib` can't import an `app` (law 1), storying a screen means lifting its presentational view into `libs/ui` — which is the point.) See `docs/design.md`.

### 3.1 SEO/GEO — the laws (enforced)

**`@zippy/seo` is the one door** for page metadata + JSON-LD. `bun run check:seo` gates it, so a public page that drifts can't merge. Source of truth: Google's guide — read it — <https://developers.google.com/search/docs/fundamentals/ai-optimization-guide>.
...
- Every public page uses **`pageMetadata()`** (title/description/canonical/OG/twitter, from `@zippy/config`).
- Content pages emit JSON-LD via `@zippy/seo` (`organizationJsonLd`, `websiteJsonLd`, `articleJsonLd`, `faqJsonLd`, `breadcrumbJsonLd` + `<JsonLd/>`) — for **rich results**, since structured data is optional for AI but valuable in classic Search.
...
- **Analytics are consent-gated (GDPR).** `@zippy/analytics` does **not** initialize PostHog/Clarity until the user accepts `<ConsentBanner/>` — default off. Don't add a tracker that bypasses this.
- **Audit trail.** `securityEvent()` (`@zippy/analytics/events`) logs security events (sign-in wired in `libs/auth`); reuse it, don't hand-roll auth logging.
...
- Single package during dev: `bun --filter @zippy/api dev`.
...
| -------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| shared code used in 2+ places                            | a new `libs/*` package   | scaffold with the Nx generator: `nx g @nx/js:lib …` — it's born tagged `type:lib`, named `@zippy/*`, with its single `src/index.ts` door                               |
...
| a new payment provider                                   | `@zippy/payment` adapter | never inline in an app; skill: `agents/skills/wire-a-new-payment-provider`                                                                                             |
...
- **Standalone `bun --filter @zippy/<x> dev`** — bun only loads `.env.local` from the _invoking_ directory, so run **`./scripts/link-env.sh`** once. It symlinks root `.env.local` into each app/service (`ln -sf ../../.env.local <pkg>/.env.local`), keeping one source of truth. The symlinks are gitignored.
